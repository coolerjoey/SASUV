...
//sd卡记录函数
void logging(){
	if(!sys_flag.task_run){	//还没有开始水下运行
...
//		File = (void*)0;	//文件关闭后，需要将文件指针指向空，这样做会防止出现游离指针，而对整个工程造成不必要的麻烦；如：fp = NULL;
	...
	sprintf(logging_data," \r\n\
					...
					\r\n",
					//传感器数据
					Sensor_latest.ins.acc[0],Sensor_latest.ins.acc[0],Sensor_latest.ins.acc[0],
					Sensor_latest.ins.gyro[0],Sensor_latest.ins.gyro[0],Sensor_latest.ins.gyro[0],
					Sensor_latest.ins.mag.x,Sensor_latest.ins.mag.x,Sensor_latest.ins.mag.x,
					Sensor_latest.ins.euler[0],Sensor_latest.ins.euler[0],Sensor_latest.ins.euler[0],
					Sensor_latest.barometer.pressure,Sensor_latest.barometer.depth,Sensor_latest.temp,Sensor_latest.power,
					//电机数据
