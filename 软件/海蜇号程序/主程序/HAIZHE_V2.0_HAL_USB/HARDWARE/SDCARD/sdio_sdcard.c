...
void SD_check(){
	if(!sys_flag.sdcard_detected){
		SD_Init();
		//TODO:MAVLINK提示请检查sd卡
	}
}

u8 SD_FATFS_Init(){
	u32 total,free;
	static bool _fatfs_init_ok = false,_sd_init_ok=false;
	//FATFS初始化
	if(!_fatfs_init_ok){
		if(exfuns_init()){	//为fatfs相关变量申请内存				 
			printf("[ERROR] FATFS malloc SRAM failed! \r\n");
			return 1;
		}	
	}
	_fatfs_init_ok = true;
	printf("[OK] FATFS malloc SRAM  \r\n");	
	if(!_sd_init_ok){
	  	if(f_mount(fs[0],"0:",1) != FR_OK){	//挂载SD卡,内部调用SD_Init()函数
			printf("[ERROR] Mount SD card failed! \r\n");
			return 2;
		} 	
	}
	_sd_init_ok = true;
	printf("[OK] Mount SD card \r\n");		
	if(exf_getfree("0:",&total,&free)){	//得到SD卡的总容量和剩余容量
		printf("[ERROR] Get SD Card information Error! \r\n");
		return 3;
	}	
	printf("[OK] SD Total_Size: %.2f MB	 Free_Size: %.2f MB \r\n",(float)total/1024,(float)free/1024);
	return 0;
}
...
u8 SD_Init(void)
{
    ...
    if(SD_Error!=SD_OK) {
		printf("[ERROR] SD Init Fail!,error code %d \r\n",SD_Error);
		return 1;
	}
    
    SD_Error=HAL_SD_WideBusOperation_Config(&SDCARD_Handler,SDIO_BUS_WIDE_4B);//使能宽总线模式
    if(SD_Error!=SD_OK) {
		printf("[ERROR] SD WideBusOperation_Config Fail! \r\n");
		return 2;
	}
	printf("[OK] SD_card Init successful \r\n");
	sys_flag.sdcard_detected = true;
    return 0;
}

u8 SD_DeInit(){
	return(HAL_SD_DeInit(&SDCARD_Handler));
}
void HAL_SD_MspDeInit(SD_HandleTypeDef *hsd)
{
    __HAL_RCC_SDIO_CLK_DISABLE();    //使能SDIO时钟
    __HAL_RCC_DMA2_CLK_DISABLE();    //使能DMA2时钟 
    __HAL_RCC_GPIOC_CLK_DISABLE();   //使能GPIOC时钟
    __HAL_RCC_GPIOD_CLK_DISABLE();   //使能GPIOD时钟
...
void HAL_SD_MspInit(SD_HandleTypeDef *hsd)
{
    ...
    __HAL_LINKDMA(hsd, hdmarx, SDRxDMAHandler); //将接收DMA和SD卡的接收DMA连接起来
    HAL_DMA_DeInit(&SDRxDMAHandler);
...
//cnt:扇区个数	
//返回值:错误状态;0,正常;其他,错误代码;	
//SD卡读写过程能否被中断打断？-> http://www.openedv.com/forum.php?mod=viewthread&tid=89408
u8 SD_WriteDisk(u8 *buf,u32 sector,u32 cnt)
{   
    u8 sta=SD_OK;
    long long lsector=sector;
