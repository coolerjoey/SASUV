...
u16 W25QXX_TYPE;	//默认是W25Q256

//4Kbytes为一个扇区Sector
//16个扇区为1个Block
//W25Q256 容量为256Mbits,32Mbytes,共有512个Block,8192个Sector 
//最小擦除单位为1个扇区，因此若每个扇区存储一个参数，最多可存8192个参数
...
{ 
	...
	W25QXX_SPI_SetSpeed(SPI_BAUDRATEPRESCALER_2); //设置为45M时钟,高速模式
	W25QXX_TYPE=W25QXX_ReadID();	        //读取FLASH ID.
	// W25Q256及以上型号有32位地址，其他型号只有24位地址
	//一位地址指向1byte的存储空间
	//24位地址：2^24byte=16*1024*1024byte = 16Mbytes
	switch(W25QXX_TYPE){
		case W25Q256: //SPI FLASH为W25Q256
			printf("[OK] SPI Flash W25Q2xx Detected  \r\n");
			temp=W25QXX_ReadSR(3);              //读取状态寄存器3，判断地址模式
			if((temp&0X01)==0)			        //如果不是4字节地址模式,则进入4字节地址模式
			{
				W25QXX_CS=0; 			        //选中
				W25QXX_SPI_ReadWriteByte(W25X_Enable4ByteAddr);//发送进入4字节地址模式指令   
				W25QXX_CS=1;       		        //取消片选   
			}
...
//注意：如果指定地址指定的存储空间还没被写过数据，都出来的是0xff
...
{ 
	...
	*/	
	...
	{	
		...
		{
			if(W25QXX_BUF[secoff+i]!=0XFF)break;//默认保存0xff，如果想要写入的空间内已经有数据写入，先擦出整个扇区  	  
		}
		if(i<secremain)//需要擦除
		{
		...
		else//写入未结束	//要写入的字节数大于要写入扇区内部剩余空间的大小
		{
			secpos++;//扇区地址增1
			secoff=0;//偏移位置为0 	 
