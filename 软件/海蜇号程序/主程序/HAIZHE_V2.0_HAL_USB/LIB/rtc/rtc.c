...
void rtc_update(){
	...
	if(!date_set){	//收到上位机日期指令后，更新一次RTC时间
        RTC_Set_Time(vp.sys_time.hour,vp.sys_time.minute,vp.sys_time.second,RTC_HOURFORMAT12_PM);	        //设置时间 ,根据实际时间修改
		RTC_Set_Date(vp.sys_time.year-2000,vp.sys_time.month,vp.sys_time.day,vp.sys_time.week);		                    //设置日期
		date_set = true;
	}
