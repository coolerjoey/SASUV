...
void SPI_Init(SPI_TypeDef *SPI){
	if(SPI==SPI2){
		...
		hspi4.Instance=SPI4;
		hspi4.Init.Mode=SPI_MODE_MASTER;             //设置SPI工作模式，设置为主模式
		hspi4.Init.Direction=SPI_DIRECTION_2LINES;   //设置SPI单向或者双向的数据模式:SPI设置为双线模式
		hspi4.Init.DataSize=SPI_DATASIZE_8BIT;       //设置SPI的数据大小:SPI发送接收8位帧结构
		hspi4.Init.CLKPolarity=SPI_POLARITY_HIGH;    //串行同步时钟的空闲状态为高电平
		hspi4.Init.CLKPhase=SPI_PHASE_2EDGE;         //串行同步时钟的第二个跳变沿（上升或下降）数据被采样
		hspi4.Init.NSS=SPI_NSS_SOFT;                 //NSS信号由硬件（NSS管脚）还是软件（使用SSI位）管理:内部NSS信号有SSI位控制
		hspi4.Init.BaudRatePrescaler=SPI_BAUDRATEPRESCALER_4;//定义波特率预分频的值:波特率预分频值为256
		hspi4.Init.FirstBit=SPI_FIRSTBIT_MSB;        //指定数据传输从MSB位还是LSB位开始:数据传输从MSB位开始
		hspi4.Init.TIMode=SPI_TIMODE_DISABLE;        //关闭TI模式
		hspi4.Init.CRCCalculation=SPI_CRCCALCULATION_DISABLE;//关闭硬件CRC校验
		hspi4.Init.CRCPolynomial=7;                  //CRC值计算的多项式
		HAL_SPI_Init(&hspi4);//初始化
