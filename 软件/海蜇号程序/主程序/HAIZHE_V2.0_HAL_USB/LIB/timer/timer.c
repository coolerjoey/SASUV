...
void HAL_TIM_IC_MspInit(TIM_HandleTypeDef *htim)
{
   GPIO_InitTypeDef GPIO_Initure;
    ...
    HAL_NVIC_SetPriority(TIM2_IRQn,0,3);    //设置中断优先级，抢占优先级2，子优先级0
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
	if(htim==(&htim3)){
		...
		static u16 sys_ten_microtick=0;//1us计时一次 
		...
		sys_time.loop_tick++;	//系统循环时钟,10us计数
		sys_ten_microtick++;
		sys_microtick++;
	...
	if(htim==&htim2){
		if((TIM2CH4_CAPTURE_STA&0X80)==0){//还未成功捕获
			if(TIM2CH4_CAPTURE_STA&0X40){		//捕获到一个下降沿 		
				...
				TIM2CH4_CAPTURE_VAL=sys_microtick;//获取当前的捕获值.
